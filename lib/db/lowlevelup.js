...
LowlevelUp.prototype.init = function init() {
  ...
  let db;
  if (Backend.name === 'MemDB' && this.options.memDbRaw) {
    db = Backend.fromRaw(this.options.memDbRaw);
    this.options.memDbRaw = null;
  } else db = new Backend(this.location);
...
LLUOptions.prototype.fromOptions = function fromOptions(options) {
  ...
  if (options.memDbRaw != null) {
    assert(Buffer.isBuffer(options.memDbRaw),
      '`memDbRaw` must be an Buffer.');
    this.memDbRaw = options.memDbRaw;
  }
