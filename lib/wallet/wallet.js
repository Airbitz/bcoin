...
Wallet.prototype.fromOptions = function fromOptions(options) {
  ...
  if (options.masterIndex || options.masterIndex === 0) {
    assert(typeof options.masterIndex === 'number');
    this.masterIndex = options.masterIndex;
  }

  if (options.masterPath) {
    assert(HD.common.parsePath(options.masterPath, false));
    this.masterPath = options.masterPath;
  }

  if (typeof options.secureMode === 'boolean') {
...
Wallet.prototype.importMasterKey = function importMasterKey(options) {
  let key = options.master;
  let mnemonic;
  if (key) {
    if (typeof key === 'string')
  ...
  this.master = MasterKey.fromKey(key, mnemonic);
};
...
Wallet.prototype.getID = function getID() {
  ...
  const key = this.master.key.derive(index);
...
Wallet.prototype.getToken = function getToken(nonce) {
  ...
  if (this.masterIndex !== null)
     index = this.masterIndex;
  const key = this.master.key.derive(index, true);
...
Wallet.prototype._createAccount = async function _createAccount(options, passphrase) {
  ...
  if (this.watchOnly && options.accountKey) {
    ...
    if (this.masterPath) {
      key = this.master.key.derivePath(this.masterPath)
    } else {
...
Wallet.prototype.toJSON = function toJSON(unsafe) {
  return {
    ...
    master: this.master && this.master.toJSON(unsafe),
...
Wallet.prototype.getSize = function getSize() {
  ...
  size += 50;
  size += 9;
  size += this.masterPath ? this.masterPath.length : 0
...
Wallet.prototype.toRaw = function toRaw() {
  ...
  const masterIndex = typeof this.masterIndex === 'number' ? this.masterIndex : -1
  const masterPathSize = this.masterPath ? this.masterPath.length : 0

  bw.writeU32(this.network.magic);
  bw.writeU32(this.wid);
  bw.writeVarString(this.id, 'ascii');
  bw.writeFloat(masterIndex);
  bw.writeU32(masterPathSize);
  this.masterPath && bw.writeString(this.masterPath, 'ascii');
  ...
  bw.writeU8(this.secureMode ? 1 : 0);
  !this.secureMode && bw.writeVarBytes(this.master.toRaw());
...
Wallet.prototype.fromRaw = function fromRaw(data) {
  ...
  const masterIndex = br.readFloat();
  if (masterIndex !== -1) this.masterIndex = masterIndex
  const masterPathSize = br.readU32();
  const masterPath = br.readString('ascii', masterPathSize)
  if (masterPath) this.masterPath = masterPath
